...
class Linear2d(nn.Linear):
    ...
    def _load_from_state_dict(
        ...
        if weight_key in state_dict:
            # 如果确实有 weight_key，再去 reshape
            state_dict[weight_key] = state_dict[weight_key].view(self.weight.shape)
        else:
            # 如果 checkpoint 里缺少这个 key
            # 1) 如果 strict=True，则可把它加入 missing_keys
            # 2) 如果 strict=False，就只当作缺少权重，自动跳过
            if strict:
...
# =====================================================
def vmamba_tiny_s2l5(channel_first=True, num_classes=1):
    return VSSM(
...
def vmamba_small_s2l15(channel_first=True, num_classes=1):
    return VSSM(
...
def vmamba_base_s2l15(channel_first=True, num_classes=1):
    return VSSM(
...
# =====================================================
def vmamba_tiny_s1l8(channel_first=True, num_classes=1):
    return VSSM(
...
def vmamba_small_s1l20(channel_first=True,num_classes=1):
    return VSSM(
